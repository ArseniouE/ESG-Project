...
--(Script creates a function olapts.abesgreport which takes a date as parameter and populate olapts.abesgreport table)
----------------------------------------------------------------------------------------------------------------------

-- FUNCTION: olapts.abesgreport(date)

-- DROP FUNCTION olapts.abesgreport(date);

CREATE OR REPLACE FUNCTION olapts.abesgreport(
	ref_date date)
    RETURNS boolean
    LANGUAGE 'plpgsql'
    COST 100
...
begin
	...
	pl_targettablename:='abesgreport';
	
	TRUNCATE TABLE olapts.abesgreport; 
...
from olapts.abesgassessment assess
...
where assess.authorizationflag  --isapproved
	 and entity.isvalid_ and entity.isvisible_
	 and assess.isvalid_ and assess.isvisible_
...
insert into olapts.abesgreport (
select distinct on (INV_PTY_RTG_ENT_ID)	
	   RECORD_TYPE, AS_OF_DT,SRC_STM_CODE, UNQ_ASSES_ID,INV_PTY_RTG_ENT_ID, INV_PTY_RTG_OBLG_TAX_ID,INV_PTY_RTG_OBLG_CDI_CODE, INV_PTY_RTG_ESG_EV_DT, INV_PTY_RTG_ESG_STM_RTRVL_DT, INV_PTY_RTG_ESG_ASSES_DT 	  
      ,INV_PTY_APRV_RTG_APRV_DT, INV_PTY_RTG_CR_CMTE_DT, INV_PTY_RTG_NEXT_RVW_DT,INV_PTY_RTG_APRV_AHR_ID, INV_PTY_RTG_ANL_NM, INV_PTY_RTG_OVRD_RSN_TP_ID  
      ,INV_PTY_RTG_APRV_NM, INV_PTY_RTG_BSN_PRTF,INV_PTY_RTG_OVRD_DRVR,INV_PTY_RTG_TP,INV_PTY_RTG_ENVRNM_SCOR,INV_PTY_RTG_ENVRNM_ZON
      ,INV_PTY_RTG_SCL_SCOR,INV_PTY_RTG_SCL_ZON,INV_PTY_RTG_GVRNC_SCOR,INV_PTY_RTG_GVRNC_ZON,INV_PTY_RTG_ESG_SCOR  
      ,INV_PTY_RTG_ESG_ZON,INV_PTY_RTG_TP_SCTR,QUEST_ID,QUEST_VRSN,OBLG_RGSTRN_NUM,JRSCTN
from (	
...
order by  INV_PTY_RTG_ENT_ID, sourcepopulateddate_assess desc
);
...
ALTER FUNCTION olapts.abesgreport(date) OWNER TO olap;

-------------------------------------------------------------
--Check if the function was created and table was populated
-------------------------------------------------------------

--select * from olapts.abesgreport('2024-12-13')
