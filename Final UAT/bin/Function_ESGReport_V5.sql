...
select distinct on (assess.pkid_,entity.gc18, entity.cdicode) 
        	  ...
        	  ,case when assess.esubscore::decimal(19,2) > 0 then replace(concat('+', lpad(rPAD(assess.esubscore, 10, '0'), 16, '0')),'.',',')
 					when assess.esubscore::decimal(19,2) < 0 then replace(concat('-',  lpad(rPAD(assess.esubscore, 10, '0'), 16, '0')),'.',',') else null
                  end as esubscore 
        	  ...
        	  ,case when assess.ssubscore::decimal(19,2) > 0 then replace(concat('+', lpad(rPAD(assess.ssubscore, 10, '0'), 16, '0')),'.',',')
 					when assess.ssubscore::decimal(19,2) < 0 then replace(concat('-',  lpad(rPAD(assess.ssubscore, 10, '0'), 16, '0')),'.',',') else null
                  end as ssubscore 
        	  ...
        	  ,case when assess.gsubscore::decimal(19,2) > 0 then replace(concat('+', lpad(rPAD(assess.gsubscore, 10, '0'), 16, '0')),'.',',')
 					when assess.gsubscore::decimal(19,2) < 0 then replace(concat('-',  lpad(rPAD(assess.gsubscore, 10, '0'), 16, '0')),'.',',') else null 
               end as gsubscore 
        	  ...
        	  ,case when assess.totalesgscore::decimal(19,2) > 0 then replace(concat('+', lpad(rPAD(assess.totalesgscore, 10, '0'), 16, '0')),'.',',')
 					when assess.totalesgscore::decimal(19,2) < 0 then replace(concat('-',  lpad(rPAD(assess.totalesgscore, 10, '0'), 16, '0')),'.',',') else null 
        	  ...
        	  --,case when assess.questionnairetype is null and nace_lookup.value_ is null and companynacecode is null then null
        	  --      else concat(assess.questionnairetype,'/',nace_lookup.value_,'/',lpad(replace(companynacecode,'.',''),4,'0'))
